...
import { AGAIN_LINK, COMMAND_PAYGATE, LINK_PAYGATE, TICKET_NO, VERSION_PAYGATE, VPC_THEME } from './constants';
...
const OnepayPaygate = NativeModules.OnepayPaygate ? NativeModules.OnepayPaygate : new Proxy({}, {
  get() {
    throw new Error(LINKING_ERROR);
  }
});
export let OPCurrency;
(function (OPCurrency) {
  OPCurrency[OPCurrency["vnd"] = 0] = "vnd";
  OPCurrency[OPCurrency["usd"] = 1] = "usd";
})(OPCurrency || (OPCurrency = {}));
export let OPErrorCase;
...
export function op_createReturnUrlFromUrlScheme(urlScheme) {
  return `${urlScheme}://onepay/`;
}
export async function op_createPaymentUrl(paymentEntity) {
  ...
  let localeName = Platform.OS === 'ios' ? NativeModules.SettingsManager.settings.AppleLocale || NativeModules.SettingsManager.settings.AppleLanguages[0] //iOS 13
